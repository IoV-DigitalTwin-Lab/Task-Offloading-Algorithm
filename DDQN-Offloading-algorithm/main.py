...
def run():
    ...
    is_redis = args.env == 'redis'
    agent_names = REDIS_AGENTS if is_redis else DUMMY_AGENTS
    ...
    greedy_dist_agent = GreedyDistanceAgent()   # dummy env only
    ...
    for episode in range(episodes):
        ...
        if is_redis:
            # ── Redis path: single env.reset(), all agents on the SAME task ──────
            state = env.reset()
            ...
            # DDQN training
            ddqn_reward, ddqn_info = results['ddqn']
            ...
            # ── Dummy path: separate resets per agent (replayable synthetic env) ──
...
# --- PLOTTING ---
    ...
    def plot_all(ax_idx, metric_key, title):
        ...
        if is_redis:
            ax[ax_idx].plot(smooth(metrics["min_latency"][metric_key]), label='Min-Latency', color='purple', linestyle='-.')
            ax[ax_idx].plot(smooth(metrics["least_queue"][metric_key]), label='Least-Queue', color='brown', linestyle=':')
        else:
