...
def _select_action(agent, agent_name, state, mask, env, request):
    ...
    if agent_name in ("ddqn", "ddqn_no_tau", "ddqn_attention", "vanilla_dqn"):
...
def _run_single_agent_instance(
    instance_cfg,
    agent_name,
    offload_mode,
    stop_event=None,
    ...
    log_dir = os.path.join(
        Config.LOG_DIR, f"{agent_name}_{offload_mode}", f"instance_{iid}"
    )
    ...
    env = IoVRedisEnv(
        redis_db=redis_db,
        instance_id=iid,
        tau_enabled=(agent_name in ("ddqn", "ddqn_attention")),
    )
    if resume_training and not load_path:
        default_path = os.path.join(
            Config.BASE_DIR, "models", f"{agent_name}_rsu{iid}.pth"
        )
    ...
    print(
        f"[{agent_name}-{iid}] Starting | mode={offload_mode} | RSU={rsu_id} | DB={redis_db}"
    )
    ...
    if agent_name != "local":
        ...
        _MAX_WAIT = 15
        print(
            f"[{agent_name}-{iid}] Waiting for vehicle states (up to {_MAX_WAIT}s)..."
        )
        while time.time() - _wait_start < _MAX_WAIT:
            if stop_event.is_set():
                print(
                    f"[{agent_name}-{iid}] Shutdown requested during vehicle wait — exiting."
                )
                return
        ...
        while not stop_event.is_set():
            ...
            if agent_name != "local":
                for _ in range(MAX_DRAIN_PER_CYCLE):
                    ...
                    if state is None:
                        ...
                        fallback_action = (
                            env.rsu_ids.index(req_rsu) if req_rsu in env.rsu_ids else 0
                        )
                        env.write_decision(
                            request["task_id"], fallback_action, agent_name
                        )
                        pipe.hset(
                        ...
                        pipe.execute()
                        print(
                            f"[{agent_name}-{iid}] Task {request['task_id']}: "
                    ...
                    action = _select_action(
                        agent, agent_name, state, mask, env, request
                    )
            ...
            for tid in list(pending.keys()):
                if _now - pending[tid]["timestamp"] > TIMEOUT:
                    ...
                    _decision_present = (
                        "decision still in Redis"
                        if decisions_data
                        else "decision key already expired or missing"
                    )
                    print(
            ...
            if episode > 0 and episode % 10 == 0 and episode != last_logged_episode:
                ...
                print(
                    f"[{agent_name}-{iid}] [{ts}] ep={episode} | "
                ...
                if ddqn_agent is not None and offload_rewards and episode > 500:
                    ...
                    print(
                        f"  >> [{agent_name}-{iid}] Model saved | avg_reward={avg_r:.3f}"
                    )
            ...
            nothing_happened = (
                _offload_drained == 0 and len(ready) == 0 and _local_drained == 0
            )
            if nothing_happened:
    ...
    results = {
        ...
        "metrics": {
            ...
            "success_rates": _build_rolling_series(
                all_successes, window=100, transform=lambda w: sum(w) / len(w) * 100
            ),
            ...
            "offload_success_rates": _build_rolling_series(
                offload_successes, window=100, transform=lambda w: sum(w) / len(w) * 100
            ),
            "local_success_rates": _build_rolling_series(
                local_successes, window=100, transform=lambda w: sum(w) / len(w) * 100
            ),
...
def _log_episode(
    ...
    for ttype in TASK_TYPES:
        if lat_by_type[ttype]:
            writer.add_scalar(
                f"Latency/{ttype}", _running_mean(lat_by_type[ttype], 50), episode
            )
        if ene_by_type[ttype]:
            writer.add_scalar(
                f"Energy/{ttype}", _running_mean(ene_by_type[ttype], 50), episode
            )

    # Decision distribution (offloaded tasks only)
    if decision_types:
        writer.add_scalar(
            "Decision_RSU_Pct", _decision_pct(decision_types, "RSU"), episode
        )
        writer.add_scalar(
    ...
    for q in (1, 2, 3):
        filtered = [
            (s) for v, s in zip(qos_values, qos_successes) if int(round(v)) == q
        ]
        if filtered:
...
def run():
    ...
    if args.env == "redis":
        ...
        _r0 = _redis_lib.Redis(
            host=Config.REDIS_HOST,
            port=Config.REDIS_PORT,
            db=active[0]["redis_db"],
    ...
    for episode in range(episodes):
        ...
        metrics["ddqn"]["fail_reason"].append(
            "NONE" if info["success"] else info.get("reason", "DEADLINE_MISSED")
        )
        ...
        metrics["random"]["fail_reason"].append(
            "NONE" if r_info["success"] else r_info.get("reason", "DEADLINE_MISSED")
        )
        ...
        metrics["greedy_comp"]["fail_reason"].append(
            "NONE" if gc_info["success"] else gc_info.get("reason", "DEADLINE_MISSED")
        )
        ...
        metrics["greedy_dist"]["fail_reason"].append(
            "NONE" if gd_info["success"] else gd_info.get("reason", "DEADLINE_MISSED")
        )
        ...
        writer.add_scalars(
            "Rewards",
            {
                "DDQN": reward,
                "Random": r_reward,
                "Greedy_Comp": gc_reward,
                "Greedy_Dist": gd_reward,
            },
            episode,
        )
        writer.add_scalars(
        ...
        writer.add_scalars(
            ...
            {
                "DDQN": info["success"],
                "Random": r_info["success"],
                "Greedy_Comp": gc_info["success"],
                "Greedy_Dist": gd_info["success"],
            },
            episode,
        )
        writer.add_scalars(
            ...
            {
                "DDQN": info["latency"],
                "Random": r_info["latency"],
                "Greedy_Comp": gc_info["latency"],
                "Greedy_Dist": gd_info["latency"],
            },
            episode,
        )
        writer.add_scalars(
        ...
        if episode % 10 == 0:
            ...
            print(
                f"[{ts}] Ep {episode} | DDQN: {avg_ddqn:.2f} | Eps: {ddqn_agent.epsilon:.2f}"
            )
    ...
    def plot_all(ax_idx, metric_key, title):
        ax[ax_idx].plot(
            smooth(metrics["ddqn"][metric_key]),
            label="DDQN (Ours)",
            color="blue",
            linewidth=2,
        )
        ax[ax_idx].plot(
            smooth(metrics["greedy_comp"][metric_key]),
            label="Greedy-Comp",
            color="orange",
            linestyle="--",
        )
        ax[ax_idx].plot(
            smooth(metrics["greedy_dist"][metric_key]),
            label="Greedy-Dist",
            color="green",
            linestyle=":",
        )
        ax[ax_idx].plot(
            smooth(metrics["random"][metric_key]),
            label="Random",
            color="grey",
            alpha=0.3,
        )
